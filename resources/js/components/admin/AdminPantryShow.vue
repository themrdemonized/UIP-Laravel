    ...
    <div>
        ...
        <v-card>
            <v-card-title>
                ...
                <v-btn :to="{name: 'adminPantryCreate'}" class="primary">
                    Добавить кладовку
                </v-btn>
            </v-card-title>
            <v-card-subtitle>
            ...
            <v-simple-table>
                <template v-slot:default>
                    ...
                    <tr v-for="item in filteredList" :key="item.id">
                        <td>{{ item.name }}</td>
                        <td class="text-right">
                            <router-link class="crud-icon-link" :to="{name: 'adminPantryEdit', params: {id: item.id}}"><v-icon class="crud-icon-edit" size="18">mdi-pencil</v-icon></router-link>
    ...
    export default {
        ...
        computed:{
            filteredList: function(){
                ...
                return this.pantries.filter(function (elem) {
        ...
        methods:{
            fetch(){
                axios.get('/api/admin/getPantries')
                    .then(response=>{
                        this.pantries = response.data.pantries
                    })
            },
            dlt(id){
                if(window.confirm('Вы действительно хотите удалить эту кладовку?'))
                {
                    axios.delete('/api/admin/deletePantry/',{
                        params: {
                            pantryId: id
                        }
                    })

...
class FloorController extends Controller
{
    ...
    public function getFloor()
    {
        ...
        $floors = Floor::where('house_id', $floor->house_id)->where('porch', $flrPorch)->get()->sortBy('name', SORT_NATURAL)->values();
//
        foreach ($floors as $flr) {
            $flats = $flr->flatss->where('status_id', 1);
            $flr->emptyFlats = $flats->count();
        }

...
class FlatController extends Controller
{
    ...
    {
        $flats = $this->flat->with('floorr.house.object')->where('status_id', 1)->get()->toArray();
        ...
        $sheet->setCellValue('A1', 'Жилой комплекс');
        $sheet->setCellValue('B1', 'Дом');
        $sheet->setCellValue('C1', 'Этаж');
        $sheet->setCellValue('D1', 'Квартира');
        $sheet->setCellValue('E1', 'Стоимость');


        for ($i=0; $i < count($flats); $i++) {
            $sheet->setCellValue('A'.$row, $flats[$i]['floorr']['house']['object']['name']);
            $sheet->setCellValue('B'.$row, $flats[$i]['floorr']['house']['name']);
            $sheet->setCellValue('C'.$row, $flats[$i]['floorr']['name']);
            $sheet->setCellValue('D'.$row, $flats[$i]['name']);
            $sheet->setCellValue('E'.$row, $flats[$i]['coast']);
            $row++;
        }

        $writer = new Xlsx($spreadsheet);
        $writer->save('emptyFlats.xlsx');

        $file = public_path()."/emptyFlats.xlsx";
        $headers = array('Content-Type: application/pdf',);

        return Response::download($file, 'emptyFlats.xlsx',$headers);
    }

...
class BEESMS {
    ...
    function get_post_request($invars) {
        ...
        $send="POST https://".$this->hostname.$this->path." HTTP/1.0".$nn."Host: ".$this->hostname.":80".$nn.($proxy_auth?$proxy_auth:"")."Content-Type: application/x-www-form-urlencoded".$nn."Content-Length: $len".$nn."User-Agent: AISMS PHP class".$nn.$nn.$PostData;
        flush();
